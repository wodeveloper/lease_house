...
public class StrUtils {
	static final private String NULL_STRINGS = ";null;undefined";
	...
	public static boolean isNull(String s, boolean containsNullString) {
		if (s == null)
			return true;
		else if (containsNullString && NULL_STRINGS != null) {
			String[] nullStrings = NULL_STRINGS.split(";");
			for (int i = 0; i < nullStrings.length; i++) {
				if (s.equalsIgnoreCase(nullStrings[i]))
					return true;
			}
			return false;
		} else
	...
	public static boolean isNotNull(String s, boolean containsNullString) {
		return !isNull(s, containsNullString);
	}
		...
		loop1: while (true) {
			...
			while (true) {
				if (isEqualsIgnoreCase(c1, c2)) {
					if (index1 < parent.length() - 1 && index2 < child.length() - 1) {
						c1 = parent.charAt(++index1);
						c2 = child.charAt(++index2);
	...
	public static String setParamValue(String parent, String paramName, String paramValue, boolean ignoreCase, String equals, String split) {
		...
		int intPos = ignoreCase ? indexOfIgnoreCase(strParent, strSplit + paramName + strEquals) : strParent
				.indexOf(strSplit + paramName + strEquals);
		String result;
		if (intPos >= 0) {
			...
			intPos = ignoreCase ? indexOfIgnoreCase(strParent, strSplit + paramName + strSplit) : strParent.indexOf(strSplit + paramName + strSplit);
			if (intPos >= 0) {
				String leftStr = parent.substring(0, intPos);
				String rightStr = intPos + paramName.length() + strSplit.length() > parent.length() ? "" : parent.substring(intPos
						+ paramName.length() + strSplit.length());
				result = leftStr + paramName + strEquals + paramValue + strSplit + rightStr;
			} else
	...
	public static String deleteParam(String parent, String paramName, boolean ignoreCase, String equals, String split) {
		...
		if (intPos >= 0) {
			...
			intPos = ignoreCase ? indexOfIgnoreCase(strParent, strSplit + paramName + strSplit) : strParent.indexOf(strSplit + paramName + strSplit);
			if (intPos >= 0) {
				if (intPos + paramName.length() + strSplit.length() > parent.length()) {
					return parent.substring(0, intPos - 1);
				} else {
					String leftStr = parent.substring(0, intPos);
					String rightStr = parent.substring(intPos + paramName.length() + strSplit.length(), parent.length());
					return leftStr + rightStr;
				}
			} else
				return parent;
	 ...
	 * @return 返回删除参数后的串，不会修改原串
	 */
	public static String deleteParam(String parent, int paramIndex, String equals, String split) {
	 ...
	 * @return 返回删除参数后的串，不会修改原串
	 */
	public static String deleteParam(String parent, int paramIndex, String equals) {
	...
	public static Double strToDouble(String s, float defaultValue) {
		return strToDouble(s, new Double(defaultValue));
	}
	
	public static Double strToDouble(String s) {
		return strToDouble(s, new Double(0));
	}
	...
	public static String concatStr(List<String> list, String split) {
		if (list == null || list.size() == 0)
			return null;
		StringBuffer sb = new StringBuffer();
		for (int i = 0; i < list.size(); i++)
			sb.append((i == 0 ? "" : split) + list.get(i));
		return sb.toString();
	}
	...
	/**
	 * 生成新的32位的时间戳（17位）加随机数（15位）方式的ID。和guid区别有两点：纯数字、有数字大小（前17位）
	 * 
	 * @return 返回类似GUID，如：20150618140448275156080587222505
	 */
	public static String newTimestampRandomId() {
		Random rnd = new Random();
		SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmmssSSS");
		Date now = new Date();
		return df.format(now) + rnd.nextInt(9) + rnd.nextInt(9) + rnd.nextInt(9) + rnd.nextInt(9)
				+ rnd.nextInt(9) + rnd.nextInt(9) + rnd.nextInt(9) + rnd.nextInt(9)
				+ rnd.nextInt(9) + rnd.nextInt(9) + rnd.nextInt(9) + rnd.nextInt(9)
	 ...
	 *            随机字符串长度
	 * @param firstNoZero
	 *            true第一个字符不能为0，一般用于调用处需要转换为整数的，并且一定要指定位数的情况
	 * @return
	 */
	public static String newRandom(int randomLen, boolean firstNoZero) {
		String result = "";
		int noLen = randomLen > 0 ? randomLen : 6;
		Random rnd = new Random();
	 ...
	 * @return
	 */
	public static String newRandom(int randomLen) {
		return newRandom(randomLen, false);
	}
	...
	public static boolean isDateFormat(String date, String dateFormat) {
		return (strToDate(date, dateFormat) != null);
	}
	 ...
	 * @return
	 */
	public static boolean equals(String s1, String s2, boolean ignoreCase, boolean autoTrim, boolean nullEqual) {
		if (s1 == null && s2 == null)
			return nullEqual;
	 ...
	 * @return
	 */
	public static boolean equals(String s1, String s2, boolean ignoreCase, boolean autoTrim) {
		return equals(s1, s2, ignoreCase, autoTrim, false);
	}
	 ...
	 * @return
	 */
	public static boolean equalsIgnoreCase(String s1, String s2, boolean autoTrim, boolean nullEqual) {
		return equals(s1, s2, true, autoTrim, nullEqual);
	}
	 ...
	 * @return
	 */
	public static boolean equalsIgnoreCase(String s1, String s2, boolean autoTrim) {
		return equalsIgnoreCase(s1, s2, autoTrim, false);
	}
	...
	/**
	 * 格式化字符串，按占位符替换。占位符格式：{0}、{1}、...、{n}<br/>
	 * StrUtils.format("111{0}222{1}333{0}444{2}555{1}666{3}", list)
	 * 
	 * @param s
	 * @param replaces
	 * @return
	 */
	public static String format(String s, List replaces) {
		if (isNull(s) || replaces == null || replaces.size() == 0)
			return s;
		for (int i = 0; i < replaces.size(); i++)
			s = s.replaceAll("\\{" + i + "\\}", String.valueOf(replaces.get(i)));
		return s;
	}

	/**
	 * 格式化字符串，按占位符替换。占位符格式：{0}、{1}、...、{n}<br/>
	 * 示例：StrUtils.format("111{0}222{1}333{0}444{2}555{1}666{3}", new
	 * Object[]{"aaa", "bbb", "ccc", "ddd", "eee"})
	 * 
	 * @param s
	 * @param replaces
	 * @return
	 */
	public static String format(String s, Object[] replaces) {
		if (isNull(s) || replaces == null || replaces.length <= 0)
			return format(s, (List) null);
		List replaceList = new ArrayList();
		for (int i = 0; i < replaces.length; i++)
			replaceList.add(replaces[i]);
		return format(s, replaceList);
	};
	...
	public static String substr(String s, int beginIndex, int length) {
		...
		int len = s.length();
		if (beginIndex >= 0) {
			if (length < 0 || beginIndex + length >= len)
			...
			} else if (length < 0 || len + beginIndex + length >= len)
				return s.substring(len + beginIndex);
			else
				return s.substring(len + beginIndex, len + beginIndex + length);
		}
	}

	public static String substr(String s, int beginIndex) {
		return substr(s, beginIndex, -1);
	}
	...
	public static String parseUrlHost(String url) {
		if (isNotNull(url)) {
			// http://127.0.0.1/house/data --> 127.0.0.1
			String result = null;
	 ...
	 * @return
	 */
	public static int parseUrlPort(String url) {
		if (isNotNull(url)) {
			if (url.indexOf(":") == -1)
				return 80;
			else {
				...
				if (url.startsWith("http://"))
					...
					start = 8;
				String uri = url.substring(url.indexOf(":", start) + 1);
				for (int i = 0; i < uri.length(); i++) {
					String s = uri.substring(i, i + 1);
					if (s.equals("/") || s.equals("\\")) {
						result = strToInt(uri.substring(0, i), 80);
						break;
					}
				}
				if (result == 0)
					result = strToInt(uri, 80);
				return result;
			}
		} else
			return 0;
	 ...
	 * @return
	 */
	public static String parseUrlUri(String url) {
		if (isNotNull(url)) {
			// http://127.0.0.1/house/data --> 127.0.0.1
			String result = null;
			if (url.startsWith("http://"))
	...
	public static String redefineScript(String json) {
		...
		tagBegin = "<iframe>";
		tagEnd = "<\\/iframe>";
		begin = json.indexOf(tagBegin, 0);
		end = json.indexOf(tagEnd, begin + tagBegin.length());
		while (begin >= 0 && end > begin) {
			value = StringUtils.replaceOnce(value, tagBegin, "<_iframe>");
			value = StringUtils.replaceOnce(value, tagEnd, "</_iframe>");
			begin = value.indexOf(tagBegin, end);
			end = value.indexOf(tagEnd, tagBegin.length());
		}
		//
		...
		while (begin >= 0 && end > begin) {
			value = StringUtils.replaceOnce(value, tagBegin, "<_img>");
			value = StringUtils.replaceOnce(value, tagEnd, "</_img>");
			begin = value.indexOf(tagBegin, end);
			end = value.indexOf(tagEnd, tagBegin.length());
		}
		//
		...
		while (begin >= 0 && end > begin) {
			value = StringUtils.replaceOnce(value, tagBegin, "<_src>");
			value = StringUtils.replaceOnce(value, tagEnd, "</_src>");
			begin = value.indexOf(tagBegin, end);
			end = value.indexOf(tagEnd, tagBegin.length());
		}
